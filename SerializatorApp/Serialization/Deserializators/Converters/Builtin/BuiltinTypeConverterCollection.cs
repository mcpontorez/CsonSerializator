...
namespace SerializatorApp.Serialization.Deserializators.Converters.Builtin
{
    ...
    public class BuiltinTypeConverterCollection : IBuiltinTypeConverterCollection
    {
        ...
        public bool Contains(CsonReader cson) => _converters.Any(c => c.IsCanConvert(cson));

        public IBuiltinTypeConverter Get(CsonReader cson) => _converters.FirstOrDefault(c => c.IsCanConvert(cson));
    }
}